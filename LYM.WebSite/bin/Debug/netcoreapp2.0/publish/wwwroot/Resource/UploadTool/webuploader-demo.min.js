jQuery(function () {
    function e(e) {
        ...
        //liyouming update 2017
			s = o('<div class="file-panel" style="line-Height:30px"><span  class="cancel">删除</span><span class="rotateRight">右旋转</span><span class="rotateLeft">左旋转</span></div>').appendTo(a),
			i = a.find("p.progress span"),
			t = a.find("p.imgWrap"),
			r = o('<p class="error"></p>'),
            //liyouming  update 20170120
            k = function (w, z) {
    ...
    function i() {
        var e, a = "";
        "ready" === k ? a = "选中" + m + "张图片，共" + WebUploader.formatSize(h) + "。" : "confirm" === k ? (e = n.getStats(), e.uploadFailNum && (a = "已成功上传" + e.successNum + "张报告，" + e.uploadFailNum + '张报告上传失败，<a class="retry" href="#">重新上传</a>失败图片或<a class="ignore" href="#">忽略</a>')) : (e = n.getStats(), a = "共" + m + "张（" + WebUploader.formatSize(h) + "），已上传" + e.successNum + "张", e.uploadFailNum && (a += "，失败" + e.uploadFailNum + "张")), p.html(a)
    }
    function t(e) {
        var a;
        if (e !== k) {
            switch (c.removeClass("state-" + k), c.addClass("state-" + e), k = e) {
                ...
                case "uploading":
                    var myUrl = '';
                    for (var item in w) {
                        myUrl += ("&" + item + "_rotation=" + w[item].rotation);

                    }
                    n.options.server = n.options.server + myUrl;
                ...
                case "paused":
                    ...
                    break;
                case "confirm":
                    //serverUrl = k(serverUrl, { "rotation": e.rotation });

                    if (f.hide(), c.text("上传中...").addClass("disabled"), a = n.getStats(), a.successNum && !a.uploadFailNum) return void t("finish");
                    break;
                case "finish":
                    a = n.getStats();
                    if (a.successNum) {
    ...
    }, n.on("all", function (e, file, respose) {
        switch (e) {
